...
Testosterone is known to induce skeletal muscle hypertrophy (Herbst & Shalender, 2004). The goal of this report is to estimate the difference in change in testosterone levels (ng/dl) between the HIGH and LOW responders from pre (T1) to post test (T3), and based on this estimate and its uncertainty, draw inference regarding the effect.
...
Testosterone levels increased with `r round(estimate2, 1)` mg/dl from T1 to T3 in HIGH, and decreased with `r round(estimate1, 1)` mg/dl from T1 to T3 in LOW. The mean change between the groups was `r round(mdiff, 1)` mg/dl (95% CI: [`r round(ci1, 1)`, `r round(ci2, 1)`], *t*(`r round(df, 1)`) = `r round(t,2)`, *p* = `r round(p, 3)`).
...
Herbst, Karen L; Bhasin, Shalender Testosterone action on skeletal muscle, Current Opinion in Clinical Nutrition and Metabolic Care: May 2004 - Volume 7 - Issue 3 - p 271-277
